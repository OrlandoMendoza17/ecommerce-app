...
import { columns, filterColumns } from "./BrandsTable.helpers";
...
function BrandsTableInner() {
  const searchValues = useTableSearch();
  const filtersValues = useTableFilters(filterColumns, "brands-filters");
  const filters = filtersValues.getAppliedFilters();

  const baseConfig = { filters, q: searchValues.q };
  const countQuery = trpc.brands.count.useQuery(baseConfig);
  ...
  const query = trpc.brands.selectByRange.useQuery(config, options);
  const { data } = query;
